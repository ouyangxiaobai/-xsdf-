...
onLoad: function (options) { 
      ...
      if (options != null) {
       ...
       that.detailstid = options.id;
        wx.request({
          url: getApp().globalData.url +"vchengji.action?list&rows=10&page=0",
            data: {
                id:  options.id, 
            },
            header: {
            ...
            success: function (res) {
             // console.log("内容是："+res.data.rows[0]["a6"]);
                 ...
                 // content: res.data.rows[0]["a6"].replace("http://localhost:8080/caipin/", getApp().globalData.url),
                  kemu: res.data.rows[0]["a5"],
                  chengji: res.data.rows[0]["aaa"]
  ...
  xiugaiclick: function (e) { 
    ...
    wx.request({
      url: getApp().globalData.url +'chengji.action?update', //仅为示例，并非真实的接口地址
      data: {
    
        a4: that.data.chengji,//成绩
        id:that.detailstid 
